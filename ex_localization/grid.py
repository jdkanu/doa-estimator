...
# %%
def makeDoaGrid(stepDeg, fig=False):
    '''
...
def neighboursOnGrid(el_grid, az_grid):
    ...
    for iPos in range(nPos-1):
        for jPos in range(iPos+1, nPos):
            neighbour_grid[iPos, jPos] = angularDistance(el_grid[iPos], az_grid[iPos], el_grid[jPos], az_grid[jPos])
            neighbour_grid[jPos, iPos] = neighbour_grid[iPos, jPos]
